...
int main(int argc, char ** argv) 
{
	...
	bool recursive_flag = true; //Flag to set for recursive or sequential implementation of graafire algorithm
	bool dist_metric = false; //Flag to set distance metric- True: Weighted Euclidean; False: Euclidean
	double t, acum_t; 			//variables for execution time
	int t_freq = getTickFrequency();

	//Paths for the dataset

	string dataset_path = "/home/smriti/Documents/AVSA/Lab2/2020AVSALab2_datasets/2020AVSALab3_datasets/"; //SET THIS DIRECTORY according to your download
	...
	string baseline_res[8] = {"ETRI","PETS2006S1","PETS2006S4","PETS2006S5","VISOR/visor_Video00.avi","VISOR01","VISOR02","VISOR03"};
	...
	string project_root_path = "/home/smriti/Documents/AVSA/Lab2/2020AVSALab2_datasets/2020AVSALab3_datasets"; //SET THIS DIRECTORY according to your project
	...
	for (int c=0; c<NumCat; c++ )
	{
		...
		string makedir_cmd = "mkdir "+results_path + "/" + dataset_cat[c] + "/" + baseline_res[s];
		system(makedir_cmd.c_str());

		//MOG2 approach for foreground segmentation
		...
		for (;;) {
			...
			//Morphological operation on fgmask 
			...
			extractBlobs(fgmask, bloblist, connectivity,recursive_flag);
			//removeSmallBlobs(bloblist, bloblistFiltered, MIN_WIDTH, MIN_HEIGHT);
			//Classify the blobs in fgmask
			classifyBlobs(bloblist, dist_metric);
			...
			extractBlobs(sfgmask, sbloblist, connectivity,recursive_flag);
			...
			classifyBlobs(sbloblist, dist_metric);
			...
			Mat bt= paintBlobImage(frame,bloblist, true); 	//Classified Blobs
			Mat bf = paintBlobImage(frame,bloblist, false);	//Detected Blobs
			Mat bs_t = paintBlobImage(frame,sbloblist, true);//Stationary Classified Blobs
			Mat bs_f= paintBlobImage(frame,sbloblist, false);//Stationary Detected Blobs
			ShowManyImages(title, 6, frame, fgmask, sfgmask,bf, bt, bs_t);

			//To save results in the folder
			stringstream it_ss;
			it_ss << setw(6) << setfill('0') << it;
			string outFile=results_path + "/" + dataset_cat[c]+ baseline_res[s]+"/"+"out"+ it_ss.str() +".png";
