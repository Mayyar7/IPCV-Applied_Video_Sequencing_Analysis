...
int extractBlobs(Mat fgmask, std::vector<cvBlob> &bloblist, int connectivity,bool recursive_flag);
...
//Overloaded functions for Grassfire algroithm implementation
void neighbour_check(Mat& image, Mat& mask, std::vector<cv::Point>& neighbour, cv::Point point, int connectivity);
void neighbour_check(Mat& image, Mat& mask, cv::Point point, int connectivity);
#endif
