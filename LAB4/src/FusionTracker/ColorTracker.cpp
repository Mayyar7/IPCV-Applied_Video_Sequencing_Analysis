...
ColorTracker::ColorTracker(Rect bbox, int feature, int stride, int bins, int numCandidates)
{
	...
	calc_bounds_candidates();					//Calculate the neighbourhood region
}
...
//Process function for color based histogram tracking
Rect ColorTracker::start(Mat current_frame) {
	...
	//ShowManyImages("Visualisation", 4, current_frame,fin_cand, get_feature(last_obs), histImage_can);

	//Histogram Comparison for  model and candidate
	putText(histImage_mod,"Model Histogram", cv::Point(10,25),FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(255, 255, 255),3,8); 	 //text in red
	putText(histImage_can,"Candidate Histogram", cv::Point(10,25),FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(255, 255, 255),3,8); //text in red

	ShowManyImages("Histogram Comparison", 2,histImage_mod, histImage_can);
	return last_obs;
}
