...
void KalmanFilterOp::Start(cv::Mat frame,std::vector<Point>  &centerList, std::vector<Point> &predictedList, std::vector<Point>  &correctedList,std::vector<Point>  &measureList, cvBlob blob, int it)
{
		...
		cv::putText(detected_blob, "Detected Blob", Point2f(20, 40), FONT_HERSHEY_DUPLEX, 0.5, Scalar(0, 255, 255), 2);

		Mat trajectory= paintFinalTrajectory(frame, centerList);
		cv::putText(trajectory, "Trajectory detected", Point2f(20, 40), FONT_HERSHEY_DUPLEX, 0.5, Scalar(255, 0, 0), 2);
		...
		cv::putText(intermediate, str1, Point2f(20, 50), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0,255, 0), 2);
		cv::putText(intermediate, str2, Point2f(20, 100), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255, 0, 0), 2);
		char str[200];
		snprintf(str,200,"Frame No:%d", it);
		cv::putText(frame, str, Point2f(20, 40), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255, 255, 255), 2);
